...
| Backend regression suite | Backend business logic, API behavior, and persistence flows in `backend/tests` | `python -m coverage run -m pytest tests` from `backend/`; Backend CI equivalent | Passed locally on 2026-07-19: 174 tests passed | `backend/tests` and `.github/workflows/backend-ci.yml` |
| Backend event-photo suite | Owner-checked upload/retrieval, image normalization, thumbnail generation, replacement/deletion compensation, local storage, and S3 adapter behavior | `python -m pytest tests/test_event_photos.py tests/test_photo_storage.py -q` from `backend/`; included in the full backend suite | Included in the 2026-07-19 full backend suite | `backend/tests/test_event_photos.py`; `backend/tests/test_photo_storage.py` |
| Backend Sprint 5 style forwarding | Verifies style parameter propagation from POST /chat/ask to ask_deepseek() mock | `python -m pytest tests/test_chat_ask.py -k "style" -q` from `backend/`; included in the full backend suite | Included in the 2026-07-19 full backend suite | `backend/tests/test_chat_ask.py::test_chat_ask_forwards_style_parameter_to_ask_deepseek`; `backend/tests/test_chat_ask.py::test_chat_ask_forwards_default_style_when_not_provided` |
...
| Android unit tests | Android repository, mapping, model, session-restore, local chat history, notification state, style forwarding, and voice recording behavior covered by JVM unit tests | `.\gradlew.bat :app:testDebugUnitTest --no-daemon` locally; GitHub Actions `Android CI` workflow | Passed in CI on 2026-07-19 | `.github/workflows/android-ci.yml`; `app/src/test/java/com/lamba/app/network/ChatRepositoryTest.kt`; `app/src/test/java/com/lamba/app/network/NotificationViewedStateTest.kt`; `app/src/test/java/com/lamba/app/network/HistoryRecordEventMapperTest.kt`; `app/src/test/java/com/lamba/app/network/StatsModelTest.kt`; `app/src/test/java/com/lamba/app/SessionRestoreNavigatorTest.kt`; `app/src/test/java/com/lamba/app/chat/LocalChatHistoryRepositoryTest.kt`; `app/src/test/java/com/lamba/app/chat/LocalChatTitleTest.kt`; `app/src/test/java/com/lamba/app/chat/VoiceRecordingStateTest.kt`; `app/src/test/java/com/lamba/app/ProfileFormValidatorTest.kt` |
...
7. Open Profile, select a communication style (Friendly/Selfish/Pragmatic), confirm the selection persists across app restart.
8. Open Profile, select an avatar image, confirm it displays in the circular frame and persists across app restart.
9. Open Profile, change vehicle brand/model, confirm the selection persists.
10. Open Notifications screen, confirm recommendations appear and can be marked as read.
